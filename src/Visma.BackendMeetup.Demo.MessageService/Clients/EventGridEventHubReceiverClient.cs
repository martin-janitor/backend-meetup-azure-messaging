...
public class EventGridEventHubReceiverClient : IAsyncDisposable
{
    private readonly EventHubConsumerClient _consumerClient;
    private readonly ILogger<EventGridEventHubReceiverClient> _logger;
    private readonly EventGridEventHubOptions _options;

    public EventGridEventHubReceiverClient(
        IOptions<EventGridEventHubOptions> options,
        TokenCredential tokenCredential,
        ILogger<EventGridEventHubReceiverClient> logger)
    {
        ...
        {
            throw new ArgumentException("Event Grid Event Hub fully qualified namespace is required", nameof(options));
        }

        if (string.IsNullOrEmpty(_options.Name))
        {
            throw new ArgumentException("Event Grid Event Hub name is required", nameof(options));
        }
        ...
        _logger.LogInformation("EventGrid EventHub receiver client initialized with namespace: {Namespace}, hub: {Hub}, consumer group: {ConsumerGroup}, using RBAC",
